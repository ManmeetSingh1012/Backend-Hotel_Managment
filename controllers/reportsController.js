...
export const exportPaymentModeReportCSV = async (req, res) => {
  try {
    ...
    // Calculate totals
    const todayTotal = csvData.reduce((sum, item) => sum + item['Today Amount'], 0);
    const monthTotal = csvData.reduce((sum, item) => sum + item['This Month Amount'], 0);
    ...
    const fileName = `payment-mode-report-${hotelId}-${now.toISOString().split('T')[0]}.csv`;
    ...
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      message: process.env.NODE_ENV === 'development' ? error.message : 'Failed to export payment mode report CSV'
    });
  }
};
...
export const generateCustomerBill = async (req, res) => {
  try {
    ...
    const checkoutDate = guestRecord.checkoutDate ? new Date(guestRecord.checkoutDate) : new Date();
    ...
    const dailyBreakdown = dateRange.map(date => {
      ...
      const dayTransactions = transactions.filter(transaction => {
        const transactionDate = new Date(transaction.paymentDate).toISOString().split('T')[0];
      ...
      dayExpenses.forEach(expense => {
        ...
        if (expense.expenseType === 'food' && expense.foodOrders) {
          ...
          otherExpenses.push({
            ...
            description: `${expense.expenseType.charAt(0).toUpperCase() + expense.expenseType.slice(1)} expense`
          });
        }
      });
    ...
    const totalPayments = transactions.reduce((sum, transaction) => sum + parseFloat(transaction.amount), 0);
    ...
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      message: process.env.NODE_ENV === 'development' ? error.message : 'Failed to generate customer bill'
    });
  }
};
...
export const exportGuestRecordsByHotelCSV = async (req, res) => {
  try {
    ...
    const guestRecords = await GuestRecord.findAll({
      where: {
        hotelId,
        checkinDate: { [Op.and]: [{ [Op.ne]: null }, { [Op.lte]: targetDate }] },
    ...
    const expenses = await GuestExpense.findAll({
      where: {
        bookingId: { [Op.in]: guestIds },
        [Op.and]: [
          sequelize.where(sequelize.fn("DATE", sequelize.col("GuestExpense.createdAt")), targetDate),
        ],
      },
    ...
    transactions.forEach((t) => {
      if (!transactionsByBooking[t.bookingId]) transactionsByBooking[t.bookingId] = [];
    ...
    expenses.forEach((e) => {
      ...
      if (!expensesByBooking[e.bookingId][e.expenseType]) expensesByBooking[e.bookingId][e.expenseType] = 0;
      expensesByBooking[e.bookingId][e.expenseType] += parseFloat(e.amount || 0);
    });
    ...
    for (const record of guestRecords) {
      ...
      const guestBills = await GuestRecord.findAll({
        ...
        where: {
          id: record.id,
          checkinDate: { [Op.and]: [{ [Op.ne]: null }, { [Op.lte]: targetDate }] },
          [Op.or]: [{ checkoutDate: null }, { checkoutDate: { [Op.lte]: targetDate } }],
        },
        raw: true,
      });
      ...
      const guestFoodExpenses = await GuestExpense.findAll({
        ...
        where: {
          bookingId: record.id,
          createdAt: { [Op.gte]: checkinDate, [Op.lte]: sequelize.literal(`'${targetDate} 23:59:59'`) },
        },
        raw: true,
      });
      ...
      pendingAmountMap[record.id] = Math.max(0, totalBill + totalFoodExpenses - totalPayments);
    }
  ...
  paymentCsvData.push({
    ...
    // Convert both tables
    const guestCsv = await json2csv(guestCsvData, { keys: Object.keys(guestCsvData[0]) });
    const paymentCsv =
      paymentCsvData.length > 0
        ? await json2csv(paymentCsvData, { keys: Object.keys(paymentCsvData[0]) })
        : "No Payments Found";
    ...
    res.status(500).json({
      success: false,
      error: "Internal server error",
      message: process.env.NODE_ENV === "development" ? error.message : "Failed to export guest records CSV",
    });
  }
};